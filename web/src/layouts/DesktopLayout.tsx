...
export function DesktopLayout({ app }: { app: ReturnType<typeof import('../hooks/useAppState').useAppState> }) {
    ...
    const virtualizer = useVirtualizer({
        ...
        estimateSize: () => isWebPreview() ? 72 : 120, // Estimated height of a StoryCard
        overscan: 5,
    });
            ...
            >
                ...
                <div className="flex items-center h-full gap-3 flex-1 min-w-0" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
                    <nav className="h-full flex items-center gap-1.5 shrink-0">
                        ...
                        {/* Articles Dropdown Menu (Relocated from Tab Bar) - Desktop only */}
                        {!isWebPreview() && (
                    ...
                    {/* Inline Filter toolbar for web preview */}
                    {isWebPreview() && (
                        <>
                            <button 
                                    ...
                                    {virtualItems.map((virtualItem) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`absolute top-0 left-0 w-full ${isWebPreview() ? 'px-3 py-1' : 'px-6 py-2.5'}`}
                                                style={{
